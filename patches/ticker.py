...
def _iter_recent_jsonl(projects_dir: str, mtime_cutoff: float) -> Iterator[str]:
    ...
    Yields newest files first and stops after 30 MB to prevent MemoryError
    when the projects directory has hundreds of recent session files.
    """
    if not os.path.isdir(projects_dir):
        return
    _TICKER_BUDGET = 30 * 1024 * 1024  # 30 MB -- ticker only needs last 15 min
    _candidates: list = []
    for path in glob.glob(os.path.join(projects_dir, "*", "*.jsonl")):
        ...
        try:
            ...
            continue
        _candidates.append((mtime, fsize, path))
    _candidates.sort(reverse=True)  # newest first
    _bytes = 0
    for _mt, _fs, path in _candidates:
        if _bytes > _TICKER_BUDGET:
            break
        _bytes += _fs
        yield path
