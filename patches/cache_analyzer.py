...
def analyze_cache_opportunities(
    ...
    # Memory guard: same pattern as collector -- newest-first, 80 MB budget
    _CA_BUDGET = 80 * 1024 * 1024
    _ca_cands: list = []
    for jsonl_path in glob.glob(os.path.join(projects_dir, "*", "*.jsonl")):
        ...
        try:
            ...
            continue
        _ca_cands.append((mtime, fsize, jsonl_path))
    _ca_cands.sort(reverse=True)
    _ca_bytes = 0
    for _ca_i, (_ca_mt, _ca_fs, jsonl_path) in enumerate(_ca_cands):
        if _ca_bytes > _CA_BUDGET:
            break
        _ca_bytes += _ca_fs
        if _ca_i > 0 and _ca_i % 20 == 0:
            gc.collect()
